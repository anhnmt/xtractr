...
type XFile struct {
    ...
    Passwords []string
    // Includes is a list of files to include. If empty, all files are included.
    Includes []string
    // Excludes is a list of files to exclude. If empty, all files are included.
    Excludes []string
}
...
func inList(fileName string, list []string) bool {
    for _, file := range list {
        if strings.EqualFold(filepath.Base(fileName), filepath.Base(file)) {
            return true
        }
    }
